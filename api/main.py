...
api_description = """
Bienvenu sur mon API movieLens
...
# --- Dépendence pour la session de base de données ---
...
# --- Tester l'état de santé de notre API ---

@app.get(
    "/",
    summary=" Vérification de la santé de l'API",
    description="Vérifie qi l'API fonctionne correctement.",
...
def read_movie(
    ...
    if movie is None:
        raise HTTPException(
            status_code=404,
            detail=f"Film portant l'identifiant {movie_id} n'a pas été trouvé"
        )
    return movie


# Endpoint pour obtenir une liste des films (avec pagination et filtre facultatifs title, genre, skip, limit)

@app.get(
    "/movies",
    summary="Lister les films",
    description="Retourne une liste de fimls avec pagination et filtres optionnels par titre ou genre.",
    response_description="Lste de films",
...
@app.get(
    ...
    description="Retourne l'évaluation d'un utilisateur pour un film donnée.",
...
def read_rating(
    ...
    if rating is None:
        raise HTTPException(
            status_code=404,
            detail=f"Aucune évaluation trouvé pour l'utilsateur {user_id} et le fiml {movie_id}"
        )
    return rating

# Endpoint pour obtenir une liste d'évaluations avecc filtre
...
@app.get(
    ...
    response_description="Liste les évaluations",
...
def list_ratings(
    skip: int = Query(0, ge=0, description="Nombre de résultats à ignorer"),
    limit: int = Query(100, le=1000, description="Nombre max de résultats à retourner"),
    movie_id: Optional[int]= Query(None, description="Filtrer par ID film"),
...
# Endpoint pour récupérer un tag pour un utilisateur et un film, avec le tete du tag
